//+------------------------------------------------------------------+
//| mar_REV_05_v1_envelope_dinamico_atr.mq5                          |
//| Grupo: REVERSAO_MEDIA                                            |
//| Versao: v1 | Timeframe: M15 / M30                               |
//| Descricao: Envelope dinamico = MME21 +/- N*ATR. Preco toca      |
//|            banda externa + candle de confirmacao contrario.      |
//|            Alvo: retorno a MME21. Stop: ATR x1.5.               |
//+------------------------------------------------------------------+
#property copyright "mar_REV_05_v1_envelope_dinamico_atr"
...
//--- Estrategia
input int    PeriodoMME    = 21;
input int    PeriodoATR    = 14;
input double MultEnvelope  = 2.0;   // distancia envelope = MultEnvelope * ATR
input double MultStop      = 1.5;
input double PctCorpoConf  = 40.0;  // % corpo minimo do candle de confirmacao
input int    HoraIni       = 9;
input int    MinIni        = 30;
...
//+------------------------------------------------------------------+
int OnInit(){
   hMME = iMA(NULL,0,PeriodoMME,0,MODE_EMA,PRICE_CLOSE);
   hATR = iATR(NULL,0,PeriodoATR);
   if(hMME==INVALID_HANDLE || hATR==INVALID_HANDLE)
...
void OnTick(){
   ...
   // Indicadores
   double arrMME[1], arrATR[1];
   ArraySetAsSeries(arrMME,true); CopyBuffer(hMME,0,1,1,arrMME);
   ArraySetAsSeries(arrATR,true); CopyBuffer(hATR,0,1,1,arrATR);
   double mme21=arrMME[0], atr=arrATR[0];

   double envSup=mme21+MultEnvelope*atr;
   double envInf=mme21-MultEnvelope*atr;

   double open1 =iOpen(NULL,0,1);
   double close1=iClose(NULL,0,1);
   double high1 =iHigh(NULL,0,1);
   double low1  =iLow(NULL,0,1);
   double range =(high1-low1>0) ? high1-low1 : 1;
   double corpo =MathAbs(close1-open1);
   double pctCorpo=corpo/range*100.0;
